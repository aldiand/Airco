...
public class GeofenceBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
            if(prefs.getBoolean(MainActivity.AUTO_ON_PREF_KEY, false)) return;
            AircoUtils.turnOn(context);
        } else if (geofenceTransition == Geofence.GEOFENCE_TRANSITION_EXIT) {
            if(prefs.getBoolean(MainActivity.AUTO_OFF_PREF_KEY, false)) return;
            AircoUtils.turnOff(context);
        } else {
            // Log the error.
    ...
    private void sendNotification(Context context, int transitionType) {
        ...
        if (transitionType == Geofence.GEOFENCE_TRANSITION_ENTER) {
            builder.setSmallIcon(R.drawable.ic_radio_on_24dp)
                    .setContentTitle(context.getString(R.string.notif_auto_on));
        } else if (transitionType == Geofence.GEOFENCE_TRANSITION_EXIT) {
            builder.setSmallIcon(R.drawable.ic_radio_off_24dp)
                    .setContentTitle(context.getString(R.string.notif_auto_off));
        }
